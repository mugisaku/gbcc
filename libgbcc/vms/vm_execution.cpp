...
{
  auto  v = get_value_by_operand(st.operand());

  auto  sym = find_symbol(st.dst_label());

    if(sym)
    {
      store(m_bp+sym->offset(),st.dst_type_info(),v);
    }
}
...
{
  return get_value_by_name(ld.label());
}
...
print() const noexcept
{
  printf("[pc = %4" PRIi64 ","
    ...
    if(m_function_stack.size())
    {
      gbcc::print(m_function_stack.back()->name());
    }
