...
assign(const vm_line&   rhs) noexcept
{
    if(this != &rhs)
    {
      ...
      case(kind::st ): new(&m_data) vm_store(rhs.m_data.st);break;
...
assign(vm_line&&  rhs) noexcept
{
    if(this != &rhs)
    {
      ...
      case(kind::st ): new(&m_data) vm_store(std::move(rhs.m_data.st));break;
...
vm_line&
vm_line::
assign(vm_store&&  st) noexcept
{
  clear();

  m_kind = kind::st;

  new(&m_data) vm_store(std::move(st));
...
clear() noexcept
{
  ...
  case(kind::st ): std::destroy_at(&m_data.st);break;
...
print() const noexcept
{
  ...
  case(kind::st ): m_data.st.print();break;
