#ifndef libgbcc_syntax_HPP
#define libgbcc_syntax_HPP
...
public:
  ...
  syntax_token_info(const text::iterator&  it) noexcept{assign(it);}

  syntax_token_info&  operator=(const text::iterator&  it) noexcept{return assign(it);}

  syntax_token_info&  assign(const text::iterator&  it) noexcept{
    m_source = it.source();
...
syntax_token_string
{
  ...
  void       read_binary_number(text::iterator&  it) noexcept;
  void        read_octal_number(text::iterator&  it) noexcept;
  void      read_decimal_number(text::iterator&  it) noexcept;
  void  read_hexadecimal_number(text::iterator&  it) noexcept;
  void  read_floating_point_number(text::iterator&  it, uint64_t  i) noexcept;

  void  read_number_that_begins_by_zero(text::iterator&  it) noexcept;
  void  read_number(text::iterator&  it) noexcept;

  void  read_string(text::iterator&  it);
  void  read_raw_string(text::iterator&  it);

  static bool  test_head_of_identifier(char16_t  c) noexcept;
  static bool  test_body_of_identifier(char16_t  c) noexcept;

  void  step(text::iterator&  it);

public:
  syntax_token_string(){}
  syntax_token_string(const text&  src){assign(src);}

  syntax_token_string&  operator=(const text&  src){return assign(src);}

  syntax_token_string&  assign(const text&  src);
...
public:
  ...
  syntax_rule(const text&  src) noexcept{assign(src);}

  operator bool() const noexcept{return m_formula_list.size();}

  syntax_rule&  operator =(const text&  src) noexcept{return assign(src);}
  syntax_rule&  operator+=(const text&  src) noexcept{return append(src);}

  syntax_rule&  assign(const text&  src) noexcept;
  syntax_rule&  append(const text&  src) noexcept;
