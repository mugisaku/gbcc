...
public:
 ...
 ~vm_value(){unrefer();}
  ...
  constexpr const vm_type_info&  type_info() const noexcept{return m_type_info;}
...
public:
  vm_load() noexcept{}
  vm_load(vm_type_info  ti, std::u16string_view  lb) noexcept: m_type_info(ti), m_label(lb){}
...
public:
  vm_store() noexcept{}
  vm_store(vm_operand&&  o, vm_type_info  dst_ti, std::u16string_view  dst_lb) noexcept:
  m_operand(std::move(o)), m_dst_type_info(dst_ti), m_dst_label(dst_lb){}
...
public:
 ...
 ~vm_register() noexcept{clear();}
  ...
  vm_register&  assign(vm_type_info  ti, std::u16string_view  lb) noexcept;
...
public:
 ...
 ~vm_line() noexcept{clear();}
  ...
  vm_line&  assign(vm_store&&  st) noexcept;
  ...
  bool   is_store()    const noexcept{return m_kind == kind::st;}
  ...
  const vm_store&     get_store()    const noexcept{return m_data.st;}
...
vm_execution
{
  ...
  std::u16string_view  m_current_label;
...
public:
  ...
  void  process(const vm_store&  st) noexcept;
